...
const releases = JSON.parse(
  await Deno.readTextFile("data/quarto-releases.json"),
);

for (const obj of releases) {
  const { tag_name, assets } = obj;
  const macPkg = assets.filter((asset: { name: string }) =>
    asset.name.endsWith(`${osFilter}.tar.gz`)
  );
  if (macPkg.length === 0) {
    continue;
  }
  ...
  if (
    tag_name.startsWith("v0") ||
    tag_name.startsWith("v1.0") ||
    tag_name.startsWith("v1.1") ||
    tag_name.startsWith("v1.2") ||
    tag_name.startsWith("v1.3")
  ) {
    // do not download old releases
    continue;
  }
  ...
  const p = Deno.run({
    cmd: [
      "curl",
      "-L",
      macPkg[0].browser_download_url,
      "-o",
      `releases/${tag_name}.tar.gz`,
    ],
  });
...
if (!(await Deno.run({ cmd: ["which", "fdupes"] }).status()).success) {
  ...
  await Deno.run({ cmd: ["fdupes", "--recurse", "releases", "-H", "-m"] })
    .status();
